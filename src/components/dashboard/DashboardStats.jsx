import {stats, tshirts, orders} from '../../data/database';
  ...
  return (
    <div className="rounded-3xl bg-white p-5 shadow-md hover:shadow-md transition">
      <div className="flex justify-between items-start">
        ...
        <span
          className={`rounded-full px-3 py-1 text-xs font-semibold ${deltaClasses}`}
        >
          {delta}
        </span>
      </div>
...
function SalesChart() {
  ...
  const points = values
    .map(
      (v, i) => `${pad + i * step},${pad + (1 - v / max) * (height - pad * 2)}`
    )
    .join(' ');
  ...
  return (
    <div className="rounded-3xl bg-white p-6 shadow-md">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%">
        ...
        <polygon
          points={`${pad},${height - pad} ${points} ${width - pad},${
            height - pad
          }`}
          fill="url(#grad)"
        />
    ...
    <section className="rounded-3xl bg-white p-6 shadow-md">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
        {tshirts.map(item => (
          <div
            key={item.name}
            className="rounded-xl bg-white p-4 shadow-lg hover:shadow-md transition"
          >
            <img
              src={item.image}
              alt={item.name}
